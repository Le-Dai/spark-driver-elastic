...
public class ClassDependenciesUtils {
    public static Map<String, byte[]> getDependenciesBinary(Class<?> from)
            throws IOException, ClassNotFoundException {
        HashMap<String, byte[]> result = new HashMap<String, byte[]>();
        Set<Class<?>> dependencies = getDependencies(from);
        result.put(from.getName(), ByteArrayUtils.getClassBytes(from));
        dependencies.forEach(clazz -> {
            String name = clazz.getName();
            if(!name.startsWith("java") && !name.startsWith("scala") && !name.startsWith("org.apache.spark")){
                result.put(clazz.getName(), ByteArrayUtils.getClassBytes(clazz));
            }
        });
        return result;
    }
    ...
    public static Set<Class<?>> getDependencies(Class<?> from)
        ...
        int read=0;
        String resourceName = "";
        if(from.getName().indexOf("$") > 0)
            resourceName = from.getName().substring(from.getName().lastIndexOf(".") + 1, from.getName().length());
        else
            resourceName = from.getSimpleName();
        try(InputStream is=from.getResourceAsStream(resourceName + ".class")) {
            for(int r; ;read+=r) {
